...
namespace GeNa.Core
{
    ...
    public class GeNaSpawnerExtensionEditor : GeNaSplineExtensionEditor
    {
        ...
        public void UpdateSpawnerVisualisation()
        {
            ...
            var totalDimensions = dimensions * dimensions;
            if (totalDimensions != m_fitnessArray.Length)
                m_fitnessArray = new AabbTest[totalDimensions];
            ...
            for (int x = 0; x < dimensions; x++)
            {
                ...
                {
                    int index = (x * dimensions) + z;
                    AabbTest aabbTest = m_fitnessArray[index];
                    GeNaSpawnerInternal.GenerateAabbTest(spawner, out aabbTest, location);
                    m_fitnessArray[index] = aabbTest;
