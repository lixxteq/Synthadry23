                    ...
                    switch (m_selectionType)
                    {
                        ...
                        case SelectionType.Up:
                        {
                            foreach (GeNaNode node in m_selectedNodes)
                            {
                                ...
                                {
                                    if (m_splineModified == false)
                                        m_spline.RecordUndoSnapshot("Up Moved", () => { node.Up = (result - node.Position).normalized; });
                                    else
                        ...
                        case SelectionType.Scale:
                        {
                            foreach (GeNaNode node in m_selectedNodes)
                            {
                                ...
                                {
                                    if (m_splineModified == false)
                                        m_spline.RecordUndoSnapshot("Node Scale Changed", () => { node.Scale = result; });
                                    else
                                        node.Scale = result;
                                    m_spline.IsDirty = true;
                                    m_splineModified = true;
        ...
        private bool DrawNodeHandles(GeNaNode currentNode)
        {
            ...
            if (m_selectedNodes.Contains(currentNode))
            {
                if (isFirstNode || isLastNode)
                    DrawConnectedNodeHandles(currentNode, Styles.knobTexture2D, isFirstNode ? FIRST_NODE_COLOR : LAST_NODE_COLOR);
                else
