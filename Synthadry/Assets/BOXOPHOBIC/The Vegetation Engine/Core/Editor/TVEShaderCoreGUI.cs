...
public class TVEShaderCoreGUI : ShaderGUI
{
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] props)
    {
        ...
        // Used for debugging shared materials
        if (material0.HasProperty("_IsIdentifier"))
        {
            Shader.SetGlobalInt("TVE_DEBUG_Identifier", material0.GetInt("_IsIdentifier"));
        }
    ...
    void DrawDynamicInspector(Material material, MaterialEditor materialEditor, MaterialProperty[] props)
    {
        ...
        {
            var internalName = customPropsList[i].name;
            var displayName = customPropsList[i].displayName;

            displayName = TVEShaderUtils.GetPropertyDisplay(material, internalName, displayName);
