...
namespace TheVegetationEngine
{
    public class TVEShaderUtils
    {
        ...
        public static bool GetPropertyVisibility(Material material, string internalName)
        {
            ...
            if (material.HasProperty("_ColorsVariationValue") || material.HasProperty("_AlphaVariationValue") || material.HasProperty("_OverlayVariationValue"))
            {
            ...
            else
            {
                if (internalName == "_CategorySubsurface")
                    valid = false;
                if (internalName == "_EndSubsurface")
                    valid = false;
        ...
        public static string GetPropertyDisplay(Material material, string internalName, string displayName)
        {
            ...
            if (EditorGUIUtility.currentViewWidth > 500)
            {
                if (internalName == "_MainMetallicValue")
                {
                    ...
                    {
                        display = displayName + " (Mask Red)";
                    }
                }

                if (internalName == "_MainOcclusionValue")
                {
                    display = displayName + " (Mask Green)";
                }
                ...
                if (internalName == "_MainSmoothnessValue")
                {
                    if (!shaderName.Contains("Simple") && !shaderName.Contains("Vertex"))
                    {
                        display = displayName + " (Mask Alpha)";
                    }
                }

                if (internalName == "_MainMaskRemap")
                {
                     display = displayName + " (Mask Blue)";
                }
                ...
                {
                    display = displayName + " (Vertex Blue)";
                }

                if (internalName == "_VertexOcclusionRemap")
                {
                    display = displayName + " (Vertex Green)";
                }

                if (internalName == "_GradientMaskRemap")
                {
                    display = displayName + " (Height Mask)";
                }
            }
