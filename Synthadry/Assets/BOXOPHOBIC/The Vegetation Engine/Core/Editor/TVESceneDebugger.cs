    ...
    public class TVESceneDebugger : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            if (debugTypeIndex == 3)
            {
                ...
                EditorGUILayout.HelpBox("Select a material in the project view to see if any prefab is sharing the same material. Please note, only one material can be previewed at a time!", MessageType.Info);

                GUILayout.Space(10);

                StyledLegend("Untracked Materials", new Color(0.3f, 0.3f, 0.3f, 0.75f));
                StyledLegend("Shared Materials", new Color(0.33f, 0.61f, 0.81f, 0.75f));
            }
        ...
        void EnableDebugger()
        {
            if (SceneView.lastActiveSceneView != null)
            {
                ...
                if (debugTypeIndex == 3)
                {
                    ...
                    Shader.SetGlobalFloat("TVE_DEBUG_Value", debugInt);
                }
