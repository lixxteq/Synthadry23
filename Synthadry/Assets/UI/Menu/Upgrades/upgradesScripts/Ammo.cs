...
public class Ammo : MonoBehaviour
{
    ...
    public void CheckUprageLevelAmmo()
    {
        if (PlayerInventory.mainGuns[weaponNum].GetComponent<ItemObject>().levelAmmo == PlayerInventory.mainGuns[weaponNum].GetComponent<ItemObject>().maxLevelAmmo)
        {
            if (PlayerInventory.mainGuns[weaponNum].GetComponent<ItemObject>().maxLevelAmmo == 0)
            {
        ...
        else
        {
            ...
            {
                if (PlayerInventory.mainGuns[weaponNum].GetComponent<ItemObject>().levelAmmo > 0)
                {
                    DoButton.GetComponent<Button>().enabled = false;
                    DoButtonText.color = new Color(0.3f, 0.3f, 0.3f, 1);
            ...
            else if (PlayerInventory.mainGuns[weaponNum].GetComponent<ItemObject>().levelAmmo == 0)
            {
        ...
        Levels.text = PlayerInventory.mainGuns[weaponNum].GetComponent<ItemObject>().levelAmmo.ToString() + " / " + PlayerInventory.mainGuns[weaponNum].GetComponent<ItemObject>().maxLevelAmmo.ToString();
        Increase.text = "+" + IncreaseInt.ToString();

    }

    public void UpgradeLevelAmmo()
    {
        Debug.Log("up");
        ...
        PlayerInventory.mainGuns[weaponNum].GetComponent<ItemObject>().levelAmmo += 1;
        MaterialsUi.UpdateMaterialsUI();
        PlayerInventory.mainGuns[weaponNum].GetComponent<ItemObject>().maximumAmmo += IncreaseInt;
        CheckUprageLevelAmmo();
        InfoMainGun.UpdateInfo();

    }

    public void ReduceLevelAmmo()
    {
        PlayerInventory.fuel += fuel;
        PlayerInventory.cloth += cloth;
        PlayerInventory.metal += metal;
